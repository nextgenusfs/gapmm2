...
class TestUTRExtension:
    ...
    def test_select_best_utrs(self):
        ...
        best_utrs, method = select_best_utrs(utr_exons, "+")
        assert method in ["median_length", "common_exon_count"]
    ...
    def test_extend_utrs_plus_strand(self, mock_fasta):
        ...
        # Check that the gene boundaries were extended
        ...
        # Check that the original CDS coordinates were preserved
        assert "cds" in result["gene1"]
        assert result["gene1"]["cds"] == self.consensus_models["gene1"]["coords"]
    ...
    def test_extend_utrs_minus_strand(self, mock_fasta):
        ...
        assert result["gene2"]["cds"] == self.consensus_models["gene2"]["coords"]
